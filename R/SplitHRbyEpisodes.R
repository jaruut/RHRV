...
#  Returns a list with two vectors: InEpisodes and OutEpisodes
  ...
  VerboseMessage(HRVData$Verbose, 
                 paste("Splitting RR signal using episodes"))
  ...
  CheckInterpolation(HRVData)
   ...
   # Select episodes to split signal
	...
	npoints = length(HRVData$RR)
	first = head(HRVData$Beat$RR,1)
	last = tail(HRVData$Beat$RR,1)
	...
	l=list(InEpisodes=HRVData$RR[Aux==1],OutEpisodes=HRVData$RR[Aux==0])
